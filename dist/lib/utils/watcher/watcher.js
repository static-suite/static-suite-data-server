...
const initWatcher = () => {
    if (config_1.config.runMode === dataServer_types_1.RunMode.DEV &&
        ...
        if (config_1.config.postProcessor) {
            paths.push(path_1.default.dirname(config_1.config.postProcessor));
        }
        if (paths.length > 0) {
            ...
            watcher.on('ready', () => {
                ...
                watcher
                    ...
                    module_1.moduleHandler.load(filePath);
                    ...
                    paths.forEach(p => module_1.moduleHandler.removeAll(new RegExp(`^${p}`)));
                    conditionallyResetStore(filePath);
                    logger_1.logger.info(`Re-building development modules done`);
                })
                    .on('unlink', filePath => {
                    logger_1.logger.debug(`File ${filePath} removed`);
                    module_1.moduleHandler.remove(filePath);
                    conditionallyResetStore(filePath);
                    logger_1.logger.info(`Re-building development modules done`);
                });
            });
