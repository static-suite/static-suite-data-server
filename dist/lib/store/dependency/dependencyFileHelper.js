...
exports.dependencyIncludeHelper = {
    setIncludeDependencies: (filepath, json) => {
        const includes = json?.metadata?.includes;
        if (includes?.static) {
            const staticIncludes = Object.values(includes?.static);
            dependencyTagger_1.dependencyTagger.setDependency(filepath, staticIncludes);
        }
        if (includes?.dynamic) {
            const dynamicIncludes = Object.values(includes?.dynamic);
            dependencyTagger_1.dependencyTagger.setDependency(filepath, dynamicIncludes);
