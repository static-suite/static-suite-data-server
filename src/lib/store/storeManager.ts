...
import { StoreDataLeaf, Store, StoreManager } from './store.types';
import { INDEX, JSON_ITEMS, MAIN, VARIANTS } from './store.constants';
import { hookManager } from './hook';
import { store, storeDataSkeleton } from '.';
import { includeParser } from './includeParser/includeParser';
...
export const storeManager: StoreManager = {
  add: (relativeFilepath: string, options = { useCache: false }): Store => {
    ...
    pathParts.forEach((part, index) => {
      ...
      if (isFilenamePart) {
        ...
        if (leafFileContent) {
          ...
          store.data[INDEX].set(relativeFilepath, leafFileContent);
        }
      } else {
  ...
  remove: (relativeFilepath: string): Store => {
    ...
    pathParts.forEach((part, index) => {
      ...
      if (isFilenamePart) {
        ...
        store.data[INDEX].delete(relativeFilepath);
      } else {
  ...
  update: (relativeFilepath: string): Store => {
    storeManager.remove(relativeFilepath);
    storeManager.add(relativeFilepath);
    return store;
  },

  includeParse: () => {
    store.data[INDEX].forEach((fileContent: Json) => {
      storeManager.includeParseFile(fileContent);
    });

    store.data[INDEX].forEach((fileContent: Json) => {
      includeParser.dinamic.run(fileContent);
    });
    return store;
  },
  includeParseFile: (fileContent: Json) => {
